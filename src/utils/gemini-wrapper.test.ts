...
test('convertContentParts should convert text parts correctly', async () => {
  ...
  if (result.length !== 1) throw new Error('Expected 1 content item');
  const firstContent = result[0];
  if (!firstContent || firstContent.role !== 'user') throw new Error('Expected role to be "user"');
  if (firstContent.parts.length !== 1) throw new Error('Expected 1 part');
...
test('convertContentParts should handle file data', async () => {
  ...
  const firstContent = result[0];
  if (!firstContent || firstContent.parts.length !== 2) throw new Error('Expected 2 parts');
  const secondPart = firstContent.parts[1];
  if (!secondPart || !('fileData' in secondPart)) throw new Error('File data not included');
});
